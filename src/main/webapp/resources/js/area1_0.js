function showArea1_0() {
	...
	var html_string = '<div id="div_contents2"><div id="div_area1_0_pin"></div><div id="div_area1_0_flow">';
	for(var i = 2; i >= 0; i--){
...
function initJsonArea1_0() {
	$.getJSON( "./resources/area1_0_shop_list.json", function(data){
		shop_json = data;
		logNow(shop_json);
	});
}
...
function showSideDetailArea1_0(index_value) {
	...
	if(index_value == "all"){
		//ajax all
		for(var i = 0; i < 16; i++){
			html_string += '<div id="div_area1_0_side_detail' + i + '" class="div_area1_0_side_detail" onclick="javascript:setSideDetailArea1_0(' + "'" + shop_json.area1_0_shop_list[i] + + "'" + ');">' + i + '</div>';
		}
...
function setSideDetailArea1_0(json_data) {
	div_area1_0_pin.css('top', json_data.top);
	div_area1_0_pin.css('left', json_data.left);
	alert(json_data.index);
}
